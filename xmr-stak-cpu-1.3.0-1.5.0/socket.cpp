...
bool plain_socket::set_hostname(const char* sAddr)
{
	...
	if (ln >= sizeof(sAddrMb))
		return pCallback->set_socket_error("a");
	...
	if ((sPort = strchr(sAddrMb, ':')) == nullptr)
		return pCallback->set_socket_error("b");
	...
	if ((err = getaddrinfo(sAddrMb, sPort, &hints, &pAddrRoot)) != 0)
		return pCallback->set_socket_error_strerr("f", err);
	...
	if (ipv4.empty() && ipv6.empty())
	{
		...
		pAddrRoot = nullptr;
		return pCallback->set_socket_error("2");
	}
	...
	if (hSocket == INVALID_SOCKET)
	{
		freeaddrinfo(pAddrRoot);
		pAddrRoot = nullptr;
		return pCallback->set_socket_error_strerr("a");
	}
...
bool plain_socket::connect()
{
	...
	if (ret != 0)
		return pCallback->set_socket_error_strerr("f");
	else
		return true;
}
...
int plain_socket::recv(char* buf, unsigned int len)
{
	...
	if(ret == 0)
		pCallback->set_socket_error("d");
	if(ret == SOCKET_ERROR || ret < 0)
		pCallback->set_socket_error_strerr("a");
